...
export default function FurnitureIntakePanel({ workspace, onClose }) {
  ...
  const selectedLabel = useMemo(() => OPTIONS.find(([value]) => value === archetype)?.[1] ?? archetype, [archetype])
  ...
  const create = () => {
    setError('')
    try {
      const furniture = createFurnitureFromIntake({ archetype, dimensionsCm: dimensions, ownershipKey, photo: { ...file, previewUrl } })
      workspace.dispatchInteractionCommand(createCreateFurnitureCommand({
        archetype, dimensionsCm: dimensions, ownershipKey,
  ...
  return <div className="intake-backdrop" role="presentation"><section className="intake-panel" role="dialog" aria-modal="true" aria-labelledby="intake-title">
    ...
    {step === 1 && <div className="intake-body"><p>上传一张家具照片，照片只用于语义识别和当前会话预览。</p><label className="intake-upload"><input type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => chooseFile(event.target.files?.[0])} /><strong>选择本地图片</strong><small>JPG / PNG / WEBP · 最大 10 MB</small></label><div className="intake-demo-samples"><span>Demo 示例：</span><button type="button" onClick={() => chooseFile(new File(['desk-demo'], 'demo-desk.jpg', { type: 'image/jpeg' }))}>Desk</button><button type="button" onClick={() => chooseFile(new File(['chair-demo'], 'demo-office-chair.jpg', { type: 'image/jpeg' }))}>Office Chair</button><button type="button" onClick={() => chooseFile(new File(['special-demo'], 'demo-special.jpg', { type: 'image/jpeg' }))}>Special</button></div></div>}
