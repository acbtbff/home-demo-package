...
import { getFurnitureTypeLabel } from '../../presentation/furnitureLabels.js'
...
export default function FurnitureIntakePanel({ workspace, onClose }) {
  ...
  return <div className="intake-backdrop" role="presentation"><section className="intake-panel" role="dialog" aria-modal="true" aria-labelledby="intake-title">
    <header className="intake-header"><div><span className="eyebrow">添加家具</span><h2 id="intake-title">添加家具</h2></div><button type="button" onClick={onClose} aria-label="关闭">×</button></header>
    ...
    {step === 1 && <div className="intake-body"><p>选择添加方式</p><div className="intake-mode"><button className={mode === 'OWNED' ? 'active' : ''} type="button" onClick={() => setMode('OWNED')}>📷 添加已有家具</button><button className={mode === 'WISHLIST' ? 'active' : ''} type="button" onClick={() => setMode('WISHLIST')}>🛒 添加想买的家具</button></div><p>上传照片或商品截图，图片只用于语义识别和当前会话预览。</p><label className="intake-upload"><input type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => chooseFile(event.target.files?.[0])} /><strong>选择本地图片</strong><small>JPG / PNG / WEBP · 最大 10 MB</small></label><div className="intake-demo-samples"><span>示例：</span><button type="button" onClick={() => chooseFile(new File(['desk-demo'], 'demo-desk.jpg', { type: 'image/jpeg' }))}>书桌</button><button type="button" onClick={() => chooseFile(new File(['chair-demo'], 'demo-office-chair.jpg', { type: 'image/jpeg' }))}>办公椅</button><button type="button" onClick={() => chooseFile(new File(['special-demo'], 'demo-special.jpg', { type: 'image/jpeg' }))}>特殊家具</button></div></div>}
    {step === 2 && <div className="intake-body">{previewUrl && <img className="intake-preview" src={previewUrl} alt="家具照片预览" />}<p><strong>识别结果：</strong>{getFurnitureTypeLabel({ semantic: { archetype: recognition?.archetype ?? archetype } })}（置信度 {Math.round((recognition?.confidence ?? 0) * 100)}%）</p><label className="intake-field">请确认家具类型<select value={archetype} onChange={(event) => setArchetype(event.target.value)}>{OPTIONS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><p className="intake-note">识别不可用或置信度较低时，请直接手动选择。</p><button className="intake-primary" type="button" onClick={() => setStep(3)}>下一步：确认尺寸</button></div>}
    ...
    {step === 4 && <div className="intake-body intake-success">{previewUrl && <img className="intake-preview small" src={previewUrl} alt="家具照片预览" />}<h3>已加入“我的家具”</h3><p>{selectedStrategy === 'GENERATED' ? '该家具的 3D 模型待生成，当前可先保存真实尺寸。' : '系统已自动选择模型路线；真实尺寸已转换为米。'}</p>{selectedStrategy !== 'GENERATED' && <button className="intake-primary" type="button" onClick={place}>放入小屋</button>}<button type="button" onClick={onClose}>{selectedStrategy === 'GENERATED' ? '关闭' : '稍后放置'}</button></div>}
  </section></div>
}
